<?php 
	...
	class Panel
	{
		...
		public function UptadeInfoForProduct($id, $title, $short_content, $rang, $text)
		{
			DB :: $dbh -> query("UPDATE news SET title = ?, short_content = ?, rang = ?, text = ? 
								 WHERE id = ?;",
								 array($title, $short_content, $rang, $text, $id));
		}

		public function AddNewProduct($title, $short_content, $rang, $text) 
		{
			...
			DB :: $dbh -> query("INSERT INTO news (title, short_content, rang, img_1, img_2, img_3, text) 
								 VALUES ( ?, ?, ?, ?, ?, ?, ?); 
								 LIMIT 1", 
								 array($title, $short_content, $rang, $mass[1], $mass[2], $mass[3], $text));
